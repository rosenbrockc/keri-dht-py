...
class EndpointError(Exception):
    """Error when a client attempts to request a KERI DHT method that does
    not exist or is not available on the DHT server.
    """


class MethodError(Exception):
    """Error raised when a client attempts to execute a request method that
    does not exist or is not available on the DHT server.
    """